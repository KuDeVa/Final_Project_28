...
def test_register_from_main(browser, email):
    ...
    browser.find_element(By.CSS_SELECTOR, "[placeholder='Введите свой код скидки, телефон или эл.почту']").send_keys(
        email)
...
@pytest.mark.guest
@pytest.mark.parametrize("email", ["f" * 15, "", "f" * 255, "ф" * 255, "ф" * 15, 0, 1, 250, -1, 1.5, "   ",
                                   "|!@#$%^&*()-_=+`~?№;:[]{}"])
